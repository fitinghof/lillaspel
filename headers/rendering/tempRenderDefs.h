...
DirectX::XMMATRIX XMMATRIX_ViewProjMatrix_Perspective(float fov, float aspectRatio, float* cameraPos, float* lookPos, float* up)
{
	return XMMATRIX_ViewMatrix(cameraPos, lookPos, up) * XMMATRIX_ProjMatrix_Perspective(fov, aspectRatio);
}

void ConstantBufferViewProjMatrix_Perspective(MatrixContainer*& data, float fov, float aspectRatio, float* cameraPos, float* lookPos, float* up)
{

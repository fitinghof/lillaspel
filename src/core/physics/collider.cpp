...
void Collider::MakeDrawable(ID3D11Device* device)
{
	this->transform.CreateMatrixBuffer(device);
}

void Collider::DrawVisualMesh(ID3D11DeviceContext* immediateContext)
{
	if (!this->visualMesh || !this->pixelShader)
	{
		std::cout << "pixelshader or visualMesh nullptr!" << std::endl;
		return;
	}

	this->pixelShader->BindShader(immediateContext);
	this->transform.BindMatrixBuffer(immediateContext, 1);
	this->visualMesh->BindMeshBuffers(immediateContext);

	this->visualMesh->Draw(immediateContext, false);
}
